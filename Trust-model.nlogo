...
ask users
  [
    if any? out-tweetlink-neighbors
    [
      let in-positive mean [positive-comment-tweet] of out-tweetlink-neighbors
      let in-negative mean [negative-comment-tweet] of out-tweetlink-neighbors

      ifelse in-positive + in-negative < 1
      [
        set FeedbackFactor 0
      ]
      [
        set FeedbackFactor (in-positive / (in-positive + in-negative))
      ]
    ]
  ]
end
...
to Information-based
 if any? tweets
  [
    ask tweets
    [
      set tweet-accuracy [con_value * 0.005 + know * 0.5] of one-of in-tweetlink-neighbors
  ...
  [
     ...
     [
       ...
       set comment-logic [ ( con_value / (con_value + neu_value )) * 0.5 + know * 0.5] of one-of in-commentlink-neighbors
     ]
  ]
...
to overall-trust
  ...
  ;ask users [ if %trust > 1 [set %trust 1]]
  ;ask users with [openness = 1] [ if any? tweetlink-neighbors with [fake-news = 1][ set %trust 0]]
  ;ask users with [con > #conscientiousness] [
   ; if any? tweetlink-neighbors with [fake-news = 1][ set %trust informationFactor]set ovtrustdone true]
  ;ask users with [received-clarification][set %trust 0]
end
  ...
  [
    ...
    [
     hatch-comments 1 [
        ...
        create-commentlinks-to other [in-tweetlink-neighbors] of self
        create-commentlinks-to other [out-following-neighbors] of self
      ]
...
to fake-news-dissemination
  ...
  ;and count tweets / count users > 0.7
  [
...
@#$#@#$#@
<experiments>
  <experiment name="experiment-baseline" repetitions="100" runMetricsEveryStep="true">
  ...
  <experiment name="extro-10-0" repetitions="100" runMetricsEveryStep="true">
    <setup>setup-scale-free-network</setup>
    <go>start</go>
    <timeLimit steps="50"/>
    <metric>count tweets</metric>
  ...
  <experiment name="extro-20-0" repetitions="100" runMetricsEveryStep="true">
    <setup>setup-scale-free-network</setup>
    <go>start</go>
    <timeLimit steps="50"/>
    <metric>count tweets</metric>
  ...
  <experiment name="extro-30-0" repetitions="100" runMetricsEveryStep="true">
    <setup>setup-scale-free-network</setup>
    <go>start</go>
    <timeLimit steps="50"/>
    <metric>count tweets</metric>
  ...
  <experiment name="extro-40-0" repetitions="100" runMetricsEveryStep="true">
    <setup>setup-scale-free-network</setup>
    <go>start</go>
    <timeLimit steps="50"/>
    <metric>count tweets</metric>
